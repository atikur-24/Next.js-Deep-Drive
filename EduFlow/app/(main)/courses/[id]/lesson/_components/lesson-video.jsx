...
export const LessonVideo = ({ courseId, lesson, module }) => {
  ...
  function handleOnProgress(state) {
    // if (state.playedSeconds > duration * 0.9) {
    //   setEnded(true);
    // }
  }
  ...
  return (
    <>
      {hasWindow && (
        <ReactPlayer url={lesson.video_url} width="100%" height="470px" controls={true} onStart={handleOnStart} onDuration={handleOnDuration} onProgress={handleOnProgress} onEnded={handleOnEnded} />
      )}
    </>
  );
};

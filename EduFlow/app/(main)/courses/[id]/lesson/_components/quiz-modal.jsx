        ...
        <DialogContent className="sm:max-w-[95%] block">
          ...
          <div className="py-4">
            <h3 className="text-xl font-medium mb-10">
              <svg className="text-success inline" stroke-width="0" viewBox="0 0 512 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">
                <path
                  fill="currentColor"
                  stroke="currentColor"
            ...
            <span className="text-[10px] block text-end">
              <svg stroke="currentColor" fill="currentColor" stroke-width="0" version="1.1" viewBox="0 0 16 16" class="text-success inline" height="12" width="12" xmlns="http://www.w3.org/2000/svg">
            ...
            {currentQuiz?.options.map((option) => (
              <div key={option.label}>
                <input
                  ...
                  type="radio"
                  name="answer"
                  onChange={(e, quizId, quizTitle, selected) => updateAnswer(e, quizzes[quizIndex].id, quizzes[quizIndex].title, option.label)}
                  id={`option-${option.label}`}
                />
                <Label className="border border-border rounded px-2 py-3 block cursor-pointer hover:bg-gray-50 transition-all font-normal" htmlFor={`option-${option.label}`}>
                  {option.label}
                </Label>
              </div>
            ))}
          ...
          <DialogFooter className="flex gap-4 justify-between w-full sm:justify-between">
            <Button className="gap-2 rounded-3xl" disabled={quizIndex === 0} onClick={() => quizChangeHandler("prev")}>
              <ArrowLeft /> Previous Quiz
            </Button>
            <Button className="gap-2 rounded-3xl bg-green-600" onClick={submitQuiz} type="submit">
              Submit
            </Button>
            <Button className="gap-2 rounded-3xl" disabled={quizIndex >= lastQuizIndex} onClick={() => quizChangeHandler("next")}>

  ...
  return (
    <div className="flex flex-col items-center py-8 text-center">
      <div className="relative max-h-[180px] max-w-[180px] rounded-full lg:mb-8 h-[100px] w-[100px] bg-orange-600 grid place-items-center text-4xl text-white">
        {session?.user?.image ? <Image src={session?.user?.image} alt={session?.user?.name} width={90} height={90} className="rounded-full" /> : session?.user?.name.charAt(0)?.toUpperCase()}
      </div>
